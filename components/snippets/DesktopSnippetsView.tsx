...
/**
 * Unchanged desktop layout (nav rail / content / snippets sidebar) — only
 * the state now comes from useSnippetsData() instead of being local to this
 * component, so it can't drift from what MobileSnippetsView shows.
 */
            ...
            {FILTER_TABS.map((t) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === t.key ? "bg-[var(--primary)] text-white" : "text-[var(--text-dim)] hover:text-[var(--text)]"
                )}
              >
                {t.label}
              </button>
